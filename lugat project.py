...
class Dictionary:
    ...
    def find_id(self):
        ...
        project = word.cursor()
        project.execute("use dictionary")
        istrue = f"select * from lugat where id='{from_id}'"
        project.execute(istrue)
    ...
    def find_english_word(self):
        from_english_word = input("Please enter english word which do you want to see :")
        ...
        project = word.cursor()
        project.execute("use dictionary")
        istrue = f"select * from lugat where english='{from_english_word}'"
        project.execute(istrue)
        result = project.fetchall()
        if result:
            word = mysql.connector.connect(
    ...
    def find_ozbek_word(self):
        ...
        project = word.cursor()
        project.execute("use dictionary")
        istrue = f"select * from lugat where ozbek='{find_ozbek_word}'"
        project.execute(istrue)
        result = project.fetchall()
        if result:
            word = mysql.connector.connect(
            ...
            print("Bu ozbekcha soz lugatda mavjud emas")
            self.enter()
